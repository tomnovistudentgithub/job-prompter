...
//opdracht 3. vraag en output
let question = "Je koos: " + userInputDepartment +". Over welke functie wil je meer weten? Voer een getal tussen 0 en 3 in.\n" +
...
const userInputJobs = prompt(question);


if (userInputJobs !== null && !isNaN(userInputJobs) && userInputJobs >= 0 && userInputJobs <= 3) {
    const userInputJobsInt = parseInt(userInputJobs);
    const jobTitle = departments[userInputDepartment].jobs[userInputJobsInt].title;
    const jobDescription = departments[userInputDepartment].jobs[userInputJobsInt].description;
    const departmentDescription = departments[userInputDepartment].description;

    document.getElementById('role-title').textContent = `Leuk dat je meer wilt weten over het werk van een ${jobTitle}`;
    document.getElementById('role-description').textContent = `De functie ${jobTitle} houdt in: ${jobDescription}`;
